...
ObjModel.prototype.render = function(gl, uni) {
    ...
    // each sub-mesh.  Don't forget to bind to the VAO and un-bind when finished.
    
    for(let i=0; i<this.meshes.length; i++) {
        let mesh = this.meshes[i];

        gl.uniform3fv(uni.uColor, mesh.color);

        //Bind to the VAO and draw the triangles
        gl.bindVertexArray(this.triangleData.vao);
        gl.drawElements( gl.TRIANGLES, mesh.nVerts, gl.UNSIGNED_INT, 0);
        gl.bindVertexArray(null);   //un-bind the VAO
    }

    // You can draw the edges (if they are available) by just calling the renderEdges
    // method from this.triangleData, but be sure to set the color appropriately.
