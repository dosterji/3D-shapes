 ...
 *      starting index of the sub-mesh), and color.
...
ObjModel.prototype.render = function(gl, uni) {
    ...
    // Draw each sub-mesh in a loop.  Set the color uniform variable before drawing
    // each sub-mesh.  bind to the VAO and un-bind when finished.
    
    gl.bindVertexArray(this.triangleData.vao);  //Bind the vao

    for(let i=0; i<this.meshes.length; i++) {
        let mesh = this.meshes[i];

        gl.uniform3fv(uni.uColor, mesh.color);  //Set the uniform color

        //draw the triangles
        gl.drawElements( gl.TRIANGLES, mesh.nVerts, gl.UNSIGNED_INT, mesh.start*4);
    }

    gl.bindVertexArray(null);   //un-bind the VAO

    //draw the edges
    let v = vec3.fromValues(0, 0, 0);   //create float 32 array for color (black)
    gl.uniform3fv(uni.Color, v);
    this.triangleData.renderEdges(gl);
};
            ...
            if( line.length > 0 ) {
                ...
                if( command === "newmtl") {
                    ...
                    if( currentMesh ) {
                        ...
                        vec3.set( currentMesh.color, r,g,b);
                    }
                } 
                // TODO: Handle other material properties here...
            }
        }
    };
